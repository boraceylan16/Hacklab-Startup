...
class RSSCollector:
    ...
    def collect(self, feeds: list[dict]) -> tuple[list[RawArticle], dict]:
        if not FEEDPARSER_AVAILABLE:
            raise RuntimeError("feedparser is required. pip install feedparser")

        all_articles: list[RawArticle] = []
        stats = {"attempted": 0, "succeeded": 0, "failed": 0, "duplicates_skipped": 0}

        for feed_cfg in feeds:
            stats["attempted"] += 1
            try:
                articles, dupes = self._collect_feed(feed_cfg)
    ...
    def _collect_feed(self, feed_cfg: dict) -> tuple[list[RawArticle], int]:
        raw_content = self._fetch_with_retry(feed_cfg["url"])
        ...
        for entry in entries:
            ...
            if not self._is_within_window(published_at, cutoff):
                continue
    ...
    def _extract_published(self, entry) -> Optional[datetime]:
    ...
    def _is_within_window(self, published_at: Optional[datetime], cutoff: datetime) -> bool:
    ...
    def _entry_to_article(self, entry, url: str, feed_cfg: dict) -> RawArticle:
        tags = []
